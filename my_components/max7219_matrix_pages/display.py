...
integration_ns = cg.esphome_ns.namespace("max7219matrix")
...
max7219_ns = cg.esphome_ns.namespace("max7219matrix")
MAX7219MatrixPagesComponent = max7219_ns.class_(
    "MAX7219MatrixPagesComponent", cg.PollingComponent, spi.SPIDevice, display.DisplayBuffer
)
MAX7219MatrixPagesComponentRef = MAX7219MatrixPagesComponent.operator("ref")

CONFIG_SCHEMA = cv.All(
    display.FULL_DISPLAY_SCHEMA.extend(
        {
            cv.GenerateID(): cv.declare_id(MAX7219MatrixPagesComponent),
...
async def to_code(config):
    ...
    if CONF_LAMBDA in config:
        lambda_ = await cg.process_lambda(
            config[CONF_LAMBDA], [(MAX7219MatrixPagesComponentRef, "it")], return_type=cg.void
        )
