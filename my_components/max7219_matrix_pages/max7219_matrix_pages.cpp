#include "max7219_matrix_pages.h"
#include "esphome/core/log.h"
#include "esphome/core/helpers.h"
#include "esphome/core/hal.h"
#include "max7219_matrix_pages_font.h"

namespace esphome {
namespace max7219matrix {

static const char *const TAG = "max7219MatrixPages";
...
float MAX7219MatrixPagesComponent::get_setup_priority() const { return setup_priority::PROCESSOR; }

void MAX7219MatrixPagesComponent::setup() {
  ESP_LOGCONFIG(TAG, "Setting up MAX7219_MATRIX_PAGES...");
  this->spi_setup();
  this->stepsleft_ = 0;
...
void MAX7219MatrixPagesComponent::dump_config() {
  ESP_LOGCONFIG(TAG, "MAX7219MATRIXPAGES:");
...
void MAX7219MatrixPagesComponent::loop() {
  uint32_t now = millis();
...
void MAX7219MatrixPagesComponent::display() {
  uint8_t pixels[8];
...
uint8_t MAX7219MatrixPagesComponent::orientation_180_() {
...
int MAX7219MatrixPagesComponent::get_height_internal() {
  return 8 * this->num_chip_lines_;  // TO BE DONE -> CREATE Virtual size of screen and scroll
}

int MAX7219MatrixPagesComponent::get_width_internal() { return this->num_chips_ / this->num_chip_lines_ * 8; }

void HOT MAX7219MatrixPagesComponent::draw_absolute_pixel_internal(int x, int y, Color color) {
...
void MAX7219MatrixPagesComponent::send_byte_(uint8_t a_register, uint8_t data) {
  this->write_byte(a_register);  // Write register value to MAX
  this->write_byte(data);        // Followed by actual data
}
void MAX7219MatrixPagesComponent::send_to_all_(uint8_t a_register, uint8_t data) {
...
void MAX7219MatrixPagesComponent::update() {
  this->update_ = true;
  for (int chip_line = 0; chip_line < this->num_chip_lines_; chip_line++) {
...
void MAX7219MatrixPagesComponent::invert_on_off(bool on_off) { this->invert_ = on_off; };
void MAX7219MatrixPagesComponent::invert_on_off() { this->invert_ = !this->invert_; };

void MAX7219MatrixPagesComponent::turn_on_off(bool on_off) {
  if (on_off) {
    this->send_to_all_(MAX7219_REGISTER_SHUTDOWN, 1);
  } else {
...
void MAX7219MatrixPagesComponent::scroll(bool on_off, ScrollMode mode, uint16_t speed, uint16_t delay, uint16_t dwell) {
...
void MAX7219MatrixPagesComponent::scroll(bool on_off, ScrollMode mode) {
  this->set_scroll(on_off);
  this->set_scroll_mode(mode);
}

void MAX7219MatrixPagesComponent::intensity(uint8_t intensity) {
  this->intensity_ = intensity;
  this->send_to_all_(MAX7219_REGISTER_INTENSITY, this->intensity_);
}

void MAX7219MatrixPagesComponent::scroll(bool on_off) { this->set_scroll(on_off); }

void MAX7219MatrixPagesComponent::scroll_left() {
  for (int chip_line = 0; chip_line < this->num_chip_lines_; chip_line++) {
    if (this->update_) {
...
void MAX7219MatrixPagesComponent::send_char(uint8_t chip, uint8_t data) {
...
void MAX7219MatrixPagesComponent::send64pixels(uint8_t chip, const uint8_t pixels[8]) {
...
uint8_t MAX7219MatrixPagesComponent::printdigit(const char *str) { return this->printdigit(0, str); }

uint8_t MAX7219MatrixPagesComponent::printdigit(uint8_t start_pos, const char *s) {
...
uint8_t MAX7219MatrixPagesComponent::printdigitf(uint8_t pos, const char *format, ...) {
  va_list arg;
  va_start(arg, format);
  char buffer[64];
...
uint8_t MAX7219MatrixPagesComponent::printdigitf(const char *format, ...) {
  va_list arg;
  va_start(arg, format);
  char buffer[64];
  int ret = vsnprintf(buffer, sizeof(buffer), format, arg);
...
#ifdef USE_TIME
uint8_t MAX7219MatrixPagesComponent::strftimedigit(uint8_t pos, const char *format, time::ESPTime time) {
  char buffer[64];
...
uint8_t MAX7219MatrixPagesComponent::strftimedigit(const char *format, time::ESPTime time) {
